    ...
    <div v-show="type===1||type===2||type===3" class="login">
      <div class="content">
        ...
        <form>
          ...
          <div class="item-ig">
            <button type="submit" @click.prevent="enter" @submit.prevent="enter">{{ type === 1 ? '发起直播' : type === 2 ? '加入房间' : '观看点播' }}
            </button>
          </div>
        </form>
      </div>
...
export default {
  ...
  methods: {
    selectType(type) {
      if (this) return this.$message.info('此demo暂不支持h5，请移步pc观看')
      this.type = type
    ...
    async enter() {
      ...
      if (this.type === 1) return this.$message.warning('此demo暂不支持移动端推流')
    ...
    async _enterLive(option) {
      ...
      if (!roomId) return this.$message.warning('此demo暂不支持移动端推流')
    ...
    async _jumpToRoom(roomId, nickName, identity, isNew) {
      try {
        ...
        await this.$router.push({name: 'main', params: {vtype: vtypes[identity], roomId}, query})
      } catch (e) {
