...
import {IDENTITY, docCreateUUID, wait, pageIsReload, randomHashColor1} from '@/utils'
...
export default {
  name: 'MainContentDoc',
  components: { DocFileMenu, DocBoardMenu, DocOnlyMenu, PreviewDoc },
  data() {
    return {
      docs: {},
      docViewRect: null,
  ...
  methods: {
    ...
    async addNewFile(item, is_board = 1) {
      this.allComplete = false
      const { width, height } = this.docViewRect
      const elId = docCreateUUID(Number(is_board) === 1 ? 'document' : 'board')
      let options = {
        docId: item.docId,
        elId,
        id: elId,
      ...
      if (Number(is_board) === 1) {
        ...
        const backgroundColor = randomHashColor1(Math.random().toString().slice(3 ,9))
        // const backgroundColor = `#${Math.floor(Math.random() * 0xffffff).toString(16)}`
        try {
          this.$doc.createBoard({ ...options, backgroundColor, option: _item })
          this.selectContainer({ id: elId })
          this.selectDoc = {
            cid: elId,
            is_board: 2,
    ...
    delFile(item) {
      ...
      if (side_type === 'board') {
        // 白板
        _list = list.filter((el) => Number(el.is_board) === 2)
      } else {
        _list = list.filter((el) => Number(el.is_board) === 1)
      }
    ...
    async needPreview({ docId }) {
      ...
      this.previewInfo.elId = docCreateUUID('document')
      await this.$forceUpdate()
      await this.$nextTick()
    ...
    async initDoc() {
      ...
      try {
        ...
        roomApi.report(this.$store.getters.roomId, error)
      }
    },
    /**
     * 主持人页面非刷新进入重置所有文档
