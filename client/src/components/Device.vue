<template>
  <div class="device-select-container">
    ...
    <div class="footer">
      <button class="cancel" @click="_handlerDevice(false)" v-show="!mustStream">取消</button>
      <button class="success" @click="_handlerDevice(true)">确定</button>
    </div>
  </div>
</template>
...
export default {
  name: "Device",
  props: ['handlerDevice', 'rtc', 'title', 'attributes', 'mustStream'],
  components: { },
  data: () => ({
    devices: {
      audioInputDevices: [],
  ...
  methods: {
    ...
    async changePreview () {
      ...
      try {
        const opt = {
          profile: isNullConstraint ? (VhallRTC && VhallRTC.RTC_VIDEO_PROFILE_720P_4x3_M || undefined) : profile,
  ...
  watch: {
    ...
    'video': function () {
      ...
      this.changePreview('video')
    },
    'ratio': function () {
      this.changePreview('ratio')
    },
    streamId (curr, prev) {
      // this.changePreview()
    }
