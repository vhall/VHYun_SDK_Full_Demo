<template>
  <div
    ...
    :data-viewStreamType="viewStreamType"
  ...
  >
    ...
    <div class="noVideo" v-show="noVideo">
      <img src="./images/icon-视频区摄像头关闭@2x.png" alt="">
    </div>
    <!-- moreInfo -->
    ...
    <!-- DEBUG 信息 -->
    <div class="debug" style="position: absolute;pointer-events: none;" v-if="showDebug && viewStreamId">
      ...
      <br />
      type: <span class="value">{{ viewStreamType }}</span
      >, role: <span class="value">{{ user && user.identity }}</span>
      <br />
      userId: <span class="value">{{ ((user && user.userId) || '').slice(-12) }}</span>
    </div>
    <!-- 控制 -->
    <div class="control" v-show="viewStreamId && controlShow && viewStreamType === 'local'">
      <!-- 流控制操作，控制自己 -->
      <div class="control-wrap" v-if="viewStreamType === 'local'">
        <!-- 静音 -->
        <div class="control-item" @click="muteAudio"><i class="iconfont" :class="oPMicStatusClass" /></div>
        <div class="control-item" @click="muteVideo"><i class="iconfont" :class="oPVideoStatusClass" /></div>
      </div>
      <!-- 流控制操作 - 管理员 -->
      <div class="control-wrap" v-if="viewStreamType === 'local_1' && hasOp && user.userId !== $store.state.stream.masterUserId"></div>
    </div>
  </div>
</template>
...
import { IDENTITY, promisify, stringCut, wait } from '@/utils'
...
export default {
  ...
  computed: {
    viewStreamType() {
      return this.stream?.local ?? this.local ? 'local' : 'remote'
    },
    viewStreamId() {
  ...
  methods: {
    ...
    async muteAudio() {
      ...
      if (this.$store) this.$store.commit('setRemoteStreamAVStatus', { streamId, muteStream: { audio: isMute, video: !this.stream?.video } })
    },
    async muteVideo() {
      if (!this.rtc) return
      const streamId = this.viewStreamId
      ...
      if (this.$store) this.$store.commit('setRemoteStreamAVStatus', { streamId, muteStream: { audio: !this.stream?.audio, video: isMute } })
    },
    async hasStream (streamId) {
      if (!streamId) return
      const stream = getStreamInfo(this.rtc, streamId)
      return !!stream
    }
  },
  beforeDestroy() {
